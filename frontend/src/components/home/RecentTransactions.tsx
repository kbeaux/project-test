...
export function RecentTransactions() {
  ...
  return <div className="bg-white py-16">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <h2 className="text-3xl font-bold text-gray-900 mb-8">{t("recent.transactions")}</h2>
        
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6">
          {isLoading ? [...Array(8)].map((_, i) => <div key={i} className="bg-white rounded-lg shadow-md overflow-hidden animate-pulse">
              ...
              </div>) : properties.map(property => <PropertyCard key={property.id} property={property} />)}
        </div>
      </div>
    </div>;
}